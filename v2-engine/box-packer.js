    ...
    triggers.forEach(function (trigger) {
        ...
        for (var key in sectionPageMap) {
            if (sectionPageMap.hasOwnProperty(key)) {
                ...
                if (text.indexOf(sectionUpper) !== -1) {
...
function padToMultipleOf4(container, pageCount, data) {
    while (pageCount % 4 !== 0) {
        ...
        if (pageCount % 4 === 0) {
            ...
            // Try to use voice final back cover if exists
            var voice = data && data.voice ? data.voice : {};
            if (voice.finalPage && voice.finalPage.backCover) {
                backCoverHtml = '<div class="back-cover-custom">' + escapeHtml(voice.finalPage.backCover) + '</div>' +
                    '<div class="back-cover-footer"><p>LiftRPG V2 ENGINE</p></div>';
            }
