(function(){
  ...
  var INSTRUCTIONS = [
    ...
    '',
    '## Convergence Design',
    '- The boss convergenceProof must demonstrate step-by-step that the mechanism produces passwordPlaintext exactly.',
    '- Choose ONE of these convergence patterns based on which best serves the world contract. Do not default to Pattern 1 every time.',
    '',
    '### Pattern 1: Sequential Assembly',
    '- Components concatenate in week order to form the password.',
    '- Boss mechanism: apply components in sequence. Simplest and most transparent.',
    '- Best for: worlds where order and procedure are thematic (bureaucracies, rituals, timelines).',
    '',
    '### Pattern 2: Reordering Puzzle',
    '- Each week yields one component, but the correct assembly order is NOT week order.',
    '- Boss mechanism reveals the true sequence (e.g., "arrange by the floor where each was found", "sort by the date on each cipher\'s source document").',
    ...
    '- Weekly components include decoys. More values are collected than the password needs.',
    '- Boss mechanism tells the player which to keep and which to discard (e.g., "only components from weeks with anomaly tiles", "discard any value found on a locked floor").',
    '- convergenceProof must identify which components are real vs. decoys and show the filtered result equals passwordPlaintext.',
    '- The password-record page still has one box per week — decoy weeks simply have values that get discarded.',
    '- Best for: worlds where deception, filtering, or judgment is thematic (espionage, trials, audits).',
    '',
    '### Pattern 4: Dual-Source Assembly',
    '- Some components come from ciphers as usual, but others must be extracted from oracle table results or fragment content.',
    '- Boss mechanism tells the player to combine cipher-derived and narrative-derived values.',
    '- convergenceProof must document both cipher and narrative sources for each component.',
